...
const Header = ({ social }) => {
  ...
  return (
    <motion.header className="fixed top-0 md:mt-12 md:mr-12 right-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] z-20 flex items-center justify-between w-[95%] px-6 rounded-2xl bg-[#000000]">
      ...
      <Link href={"/"} className="bg-[#000000] px-3 py-6 rounded-2xl text-[#ffffff] font-bold text-2xl">
      Voisa Community
      </Link>
      <motion.div
        initial={false}
        ...
        variants={variants}
        className={`absolute top-0 right-0 mx-[24px] my-[24px]  md:-top-6 md:-right-6 w-dvw md:w-[480px] h-dvh md:h-[calc(100dvh_-_2.5rem)] bg-black ${isActive? 'visible' : 'hidden' } z-[-1000]`}
      >
        {isActive && (
...
function Button({toggleMenu }) {
  return (
    ...
    <button className="w-[50px] h-[50px] bg-[#ffffff] rounded-2xl flex items-center justify-center hover:opacity-80" 
    onClick={
      ()=>{
        toggleMenu();
      }
